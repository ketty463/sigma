...
local pcall, getgenv, next, setmetatable, Vector2new, CFramenew, Color3fromRGB, Drawingnew, TweenInfonew, stringupper, mousemoverel = 
    pcall, getgenv, next, setmetatable, Vector2.new, CFrame.new, Color3.fromRGB, Drawing.new, TweenInfo.new, string.upper, mousemoverel or (Input and Input.MouseMove)
...
local function TriggerBot()
    if Environment.TriggerBotSettings.Enabled and Environment.Locked and Environment.Locked.Character then
        ...
        if LockPart then
            ...
            -- Check if the target is within the activation radius and visible on screen
...
local function Load()
    ...
    ServiceConnections.InputBeganConnection = UserInputService.InputBegan:Connect(function(Input)
        if not Typing then
            pcall(function()
                if Input.KeyCode == Enum.KeyCode[Environment.TriggerBotSettings.TriggerKey] then
