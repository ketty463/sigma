...
if not getgenv().Captive or getgenv().Captive.Aimbot then return end
...
local Environment = getgenv().Captive.Aimbot
...
local function GetClosestPlayer()
    if not Environment.Locked then -- Only find a target if no one is locked
    ...
    elseif Environment.Locked then
        ...
        if LockedPlayer.Character and LockedPlayer.Character:FindFirstChild(Environment.Settings.LockPart) and LockedPlayer.Character:FindFirstChildOfClass("Humanoid") then
            local Vector, OnScreen = Camera:WorldToViewportPoint(LockedPlayer.Character[Environment.Settings.LockPart].Position)
            Vector = ConvertVector(Vector)
            local Distance = (UserInputService:GetMouseLocation() - Vector).Magnitude

            if Distance > RequiredDistance or not OnScreen or LockedPlayer.Character:FindFirstChildOfClass("Humanoid").Health <= 0 then
                CancelLock() -- Cancel the lock if the player is no longer valid
            end
        else
            CancelLock() -- Cancel lock if the locked player becomes invalid
        end
    end
end
...
function Environment.Functions:Exit()
	...
	getgenv().Captive.Aimbot.Functions = nil
	getgenv().Captive.Aimbot = nil
